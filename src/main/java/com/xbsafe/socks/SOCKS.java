...
public class SOCKS {
	...
	static Properties loadProperties(String file_name) {
		...
		try {
			...
			String jar = System.getProperty("user.dir") + File.separator
					+ "com.xbsafe.proxyplugin.jar";
			// 将路径实例化成文件
			File jar2 = new File(jar);
			// 将文件实例化成jar包
			JarFile jarFile = new JarFile(jar2);
			// 选取jar包要读取的配置文件
			JarEntry entry = jarFile.getJarEntry(file_name);
			// 读取到配置文件流
			InputStream input = jarFile.getInputStream(entry);
			// 看看读读 reader.readerLine()

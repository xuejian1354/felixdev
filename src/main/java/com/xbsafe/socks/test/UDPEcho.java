...
public class UDPEcho implements Runnable {
   ...
   public void send(String s) throws IOException{
      ...
      DatagramPacket packet = new DatagramPacket(s.getBytes(), s.length(), hostIP, port);
      sock.send(packet);
   }
   ...
   public void run() {
      ...
      try {
	     while(true) {
	       ...
	       System.out.println("UDP From:" + incomingData.getAddress().getHostAddress() +
   ...
   public static void doEcho(int port) throws IOException {
      ...
      System.out.println("Starting UDP echo on" + sock.getLocalAddress().getHostAddress() +
                          ":" + sock.getLocalPort());
      while(true) {
	    try {
	      sock.receive(packet);
	      sock.send(packet);
          System.out.print("UDP From: " + packet.getAddress().getHostAddress() +
        		  	":" + packet.getPort() + "\n" +
